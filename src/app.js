...
async function revogarCertificado() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById("resultadoRevogacao").innerText = `Certificado  ID ${id} revogado`;
        } else {
        ...
        console.error(error);
        document.getElementById("resultadoRevogacao").innerText = `Erro ao revogar o certificado: ${error.message}`;
